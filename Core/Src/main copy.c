...
void initChannels(void) {
    for (uint8_t i = 0; i < 4; i++) {
        ...
        channels[i].full_scale_A = 4000;  // 默认满量程补偿
        channels[i].full_scale_V = 4000;  // 默认满量程补偿
...
void setCurrent() {
    float value = channels[uiState.channelFlag].current_value;
    float fullS = channels[uiState.channelFlag].full_scale_A / 10.0f;  // 转换为与value同基数(100)
    float zeroOffset = channels[uiState.channelFlag].zero_offset_A / 10.0f;  // 转换为与value同基数(100)
    // 线性补偿：实际输出 = (设定值 - 零点偏移) / 满量程 * 0xFFFF
...
void showSettingsPage(void) {
    ...
    // 第六行：满量程(Cali)和零点偏移(Zero)
    if(channels[ch].mode == 0) {
        sprintf(str, "C:%.3f Z:%.3f", (float)channels[ch].full_scale_A / 1000, (float)channels[ch].zero_offset_A / 1000);
    } else {
    ...
    if (uiState.settingFlag == 5) {  // 满量程校准
        if(uiState.isSettingFlag)
            OLED_ReverseArea(13, 50, 30, 8);  // 高亮数值
        else
            OLED_ReverseArea(1, 50, 10, 8);   // 高亮C:
    } else if (uiState.settingFlag == 6) {  // 零点偏移
        if(uiState.isSettingFlag)
            OLED_ReverseArea(58, 50, 35, 8);  // 高亮数值
        else
            OLED_ReverseArea(46, 50, 10, 8);  // 高亮Z:
    }

    OLED_Update();
}
void showUI(void) {
    char str[50];  // 分配足够的存储空�??
    OLED_Clear();
...
void KeyScan(void) {
    ...
    if (HAL_GPIO_ReadPin(KEY1_GPIO_Port, KEY1_Pin) == GPIO_PIN_SET) {
        if (key_state1 == 0) {
            ...
            key_press_time1 = 0;
							...
							key_press_time1++;
							if (key_press_time1 >= LONG_PRESS_TIME && uiState.isSettingFlag) {
									if (uiState.settingFlag == 2) {  // 电流/电压�??
    ...
    if (HAL_GPIO_ReadPin(KEY2_GPIO_Port, KEY2_Pin) == GPIO_PIN_SET) {
        if (key_state2 == 0) {
            ...
            key_press_time2++;
						if (key_press_time2 >= LONG_PRESS_TIME && uiState.isSettingFlag) {
							if (uiState.settingFlag == 2) {  // 电流/电压�??
									if (channels[uiState.channelFlag].mode == 0) {  // 电流模式
									...
									if(uiState.isSettingFlag){
										if(uiState.settingFlag == 1){
											...
											if(channels[uiState.channelFlag].mode)
												...
												setCurrent();
											SaveCalibrationToFlash();  // 保存满量程校准到Flash
										}
											...
											if(channels[uiState.channelFlag].mode)
												...
												setCurrent();
											SaveCalibrationToFlash();  // 保存零点偏移到Flash
										}
									}
